...
global $dirfonts,$dirimg,$symboles,$fontesmath,$mp_debug;
...
if(!isset($dirfonts)) $dirfonts=$_SERVER["DOCUMENT_ROOT"]."/mathpublisher/fonts";
// or $dirfonts=dirname(__FILE__)."/phpmathpublisher/fonts";

// absolute path to the images directory
if(!isset($dirimg)) $dirimg=$_SERVER["DOCUMENT_ROOT"]."/mathpublisher/img";
// or $dirimg=dirname(__FILE__)."/phpmathpublisher/img";

$mp_debug = "debug text:<br>"; 
...
{
global $symboles, $fontesmath, $dirfonts, $mp_debug;
global $mpfont_red,$mpfont_green,$mpfont_blue;
$scale_factor = 2; # default is 2. [igh I want to be able to control the scale of the large operators e.g. sum and parenthasis, but it is not working yet.]
$texte=trim(stripslashes($texte));
switch($texte)
	{
	case '':
	$img = ImageCreate(1, max($haut,1));
	$blanc=transparent_bg($img);
	...
	$noir=ImageColorAllocate($img,$mpfont_red,$mpfont_green,$mpfont_blue);
	...
	$tmp_noir=ImageColorAllocate($tmp_img,$mpfont_red,$mpfont_green,$mpfont_blue);
	$tmp_blanc=transparent_bg($tmp_img);
	...
	$tmp_largeur = abs($tmp_dim[2] - $tmp_dim[0])*$scale_factor;
	$tmp_hauteur = abs($tmp_dim[3] - $tmp_dim[5])*$scale_factor;
	$tmp_img = ImageCreate(max($tmp_largeur,1), max($tmp_hauteur,1));
	$tmp_noir=ImageColorAllocate($tmp_img,$mpfont_red,$mpfont_green,$mpfont_blue);
	$tmp_blanc=transparent_bg($tmp_img);
	ImageFilledRectangle($tmp_img,0,0,$tmp_largeur,$tmp_hauteur,$tmp_blanc);
...
$noir=ImageColorAllocate($result,$mpfont_red,$mpfont_green,$mpfont_blue);
$blanc=transparent_bg($result);
...
$noir=ImageColorAllocate($imggauche,$mpfont_red,$mpfont_green,$mpfont_blue);
$blanc=transparent_bg($imggauche);
...
$noir=ImageColorAllocate($imgfin,$mpfont_red,$mpfont_green,$mpfont_blue);
$blanc=transparent_bg($imgfin);
...
while ($fi = readdir($handle))
	{
	$info=pathinfo($fi);
	if (isset($info["extension"]) && $fi!="." && $fi!=".." && $info["extension"]=="png" && preg_match("/^math/",$fi)) 
		{
...
$valign=$v-1000;
$label_text = htmlentities(str_replace('{prime}',"'",$text), ENT_COMPAT, 'UTF-8'); # encode so html stays valid
#$label_text = htmlentities(str_replace('{prime}',"'",$text)); # encode so html stays valid
$text = str_replace("\x80",'[euro80]',$text);
$text = str_replace("\xe282ac",'[euroU]',$text);
...
function transparent_bg($img){
	global $mpbg_red, $mpbg_green, $mpbg_blue;
	$blanc=ImageColorAllocate($img,$mpbg_red, $mpbg_green, $mpbg_blue);
	$blanc=imagecolortransparent($img,$blanc);
	return $blanc;
}

// convert #rrggb or #rgb (optional # to r,g,b integer array
function html2rgb($color){
	...
	elseif (strlen($color) == 3)
		...
		return array(0,0,0); # error returns black
