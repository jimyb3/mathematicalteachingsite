...
sourcerer_init = function() {
	...
	if (string.search('{' + sourcerer_syntax_word) != -1) {
		button.addClass('button-src-nosourcetags');
	}
	...
	if (string.search(/\[\[/g) != -1 && string.search(/\]\]/g) != -1) {
		buttonaddClass('button-src-tagstylebrackets');
	}
};
...
sourcerer_toggleSourceTags = function(add) {
	...
	if (!add && !button.hasClass('button-src-nosourcetags')) {
		button.addClass('button-src-nosourcetags');
	} else {
		string = '{' + sourcerer_syntax_word + '}\n' + string + '\n' + '{/' + sourcerer_syntax_word + '}';
		button.removeClass('button-src-nosourcetags');
	}
...
sourcerer_toggleTagStyle = function() {
	...
	if (!button.hasClass('button-src-tagstylebrackets')) {
		...
		button.addClass('button-src-tagstylebrackets');
	} else {
		button.removeClass('button-src-tagstylebrackets');
	}
