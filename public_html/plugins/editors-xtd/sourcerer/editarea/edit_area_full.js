...
}, translate: function(text, lang, mode)
{
	...
	return text;
}, translate_template: function() {return eAL.get_word_translation(EAL.prototype.translate_template.arguments[1], eAL.current_language);}, get_word_translation: function(val, lang)
{
	var i;
...
function get_css_property(elem, prop)
{
	if (document.defaultView) {
		...
		var prop = prop.replace(/-\D/gi, function(sMatch) {return sMatch.charAt(sMatch.length - 1).toUpperCase();});
...
EAL.prototype.start_resize_area = function()
{
	...
	if (editAreaLoader.isIE && editAreaLoader.isIE <= 7) {
		width -= 2;
		height -= 2;
	}
